...
function time(o) {
    if (wait == 0) {
        ...
        o.value="发送验证码";
        wait = 59;
        $("#btn").css({"background-color":"#bb8e4b"})
...
$("#submitBtn").click(function(event) {
   ...
   $.post('/user/h5/bindmobile', {data: JSON.stringify(data)}, function(data) {
        ...
        if(data.sc=="-99999"){
            ...
            //请求成功，有bindtoken则弹框，无则跳下一页
            // if(data.data!=undefined&&data.data.bindtoken!=undefined){
            //     $(".newweixin").html(data.data.newweixin);
            //     $(".oldweixin").html(data.data.oldweixin);
            //     $(".mask1").css('display', 'block');
...
$("#submitBtn1").click(function(event) {
    if($(".name").val()=="请输入您的真实姓名"){
        ...
        // console.log(data)
        $.post('/user/h5/update', {data: JSON.stringify(data)}, function(data) {
            // console.log(data)
            //判断是否关注公众号，无跳关注页，有直接进会员中心
            if(data.sc=="0"){
                if(data.data.isSubcribe){
                    window.location="/static/memberCenter/memberCenter.html"
                    
                }
                else{
                    window.location="/static/memberCenter/attention.html"
                }
            }
        }) 
    }
