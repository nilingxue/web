$(document).ready(function() {
    $.post('/member/h5/info', function(data) {
	    ...
	    if(data.sc=="0"){
            ...
            $(".integrationLeft span").html(data.data.keepgradeCondition)
         }
	        // $(".integrationLeft span").html(data.data.upgradeCondition+"<br/>"+data.data.keepgradeCondition);
	        $(".cardInfo").html(data.data.memberCode);
	        $(".integrationRightTop strong").html(data.data.points);
	        $(".integrationRightBottom strong").html(data.data.nightsCurYear); 
	        if(data.data.memberGrade=="0"){
	...
	$.post('/member/h5/points/list',{data: JSON.stringify(data)}, function(data) {
	    ...
	    if(data.sc=="0"){
	    	...
	    	if(data.data.length=="0"){
	    		// alert("aa");
	    		$(".integrationSystem").css('display', 'none');
	    	}
